...
import { UserObject } from 'convex/schema';
...
export async function apiPushUserUpdate(userData: Partial<Omit<UserObject, '_id'>>) {
...
export function validate(user: Partial<UserObject>): ValidationResult {
  ...
  if (!user.role) {
    ...
    issues = [
      ...issues,
      { message: 'Role must be "user or admin"', path: ['role'] }
    ];
  }
